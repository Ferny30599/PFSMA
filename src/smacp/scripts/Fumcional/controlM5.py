...
def callback5(data):
    ...
    time = rospy.get_time()
    with open('/home/ferny/smacp/src/smacp/scripts/plot/SMA/evasion/robot5.csv', 'a') as file:
        file.write(f"{x5h}, {y5h}, {time}\n")
...
def control():
    ...
    V = ux*math.cos(th5) + uy*math.sin(th5)
    w = -ux*math.sin(th5)/h + uy*math.cos(th5)/h
    #ux=-k1*(x5h-x4h)-k1*(x5h-x6h)
    #uy=-k1*(y5h-y4h)-k1*(y5h-y6h)
...
def F_atractiva():
    ...
    xhpos=np.array([x1h, x2h, x3h, x4h, x5h,x6h])-lx
    yhpos=np.array([y1h, y2h, y3h, y4h, y5h,y6h])-ly
    d=math.sqrt((x5h-x4h)**2+(y5h-y4h)**2)
    #if d<Qd:
...
if __name__=="__main__":
    ...
    rospy.Subscriber('/r5/s1', LaserScan, ultrasonic1_callback)
    rospy.Subscriber('/r5/s2', LaserScan, ultrasonic2_callback)
    rospy.Subscriber('/r5/s3', LaserScan, ultrasonic3_callback)
    rospy.Subscriber('/r5/s4', LaserScan, ultrasonic4_callback)
    rospy.Subscriber('/r5/s5', LaserScan, ultrasonic5_callback)
    rospy.Subscriber('/r5/s6', LaserScan, ultrasonic6_callback)
