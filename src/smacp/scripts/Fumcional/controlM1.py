...
yfin=0
tini=15
tfin=165#255#:4min #225#:3min30seg#195#:3min #165#:2min30seg #135#:2min #105#:1min30seg
...
def callback1(data):
    ...
    time = rospy.get_time()
    with open('/home/ferny/smacp/src/smacp/scripts/plot/SMA/evasion/robot1.csv', 'a') as file:
...
def trayectoria():
    ...
    with open('/home/ferny/smacp/src/smacp/scripts/plot/SMA/Campos/trayectoria.csv', 'a') as file:
...
def F_atractiva():
    ...
    #if d<Qd:
    Fatracx=-Katrac*(-dxd -1.5*(xd-x1h)+Laplaciana[agente-1].dot(xhpos.T))
    Fatracy=-Katrac*(-dyd -1.5*(yd-y1h)+Laplaciana[agente-1].dot(yhpos.T))
    #else:
    #    Fatracx=-Qd*Katrac*(-2*(xd-x1h)+Laplaciana[agente-1].dot(xhpos.T))/d
    #    Fatracy=-Qd*Katrac*(-2*(yd-y1h)+Laplaciana[agente-1].dot(yhpos.T))/d
