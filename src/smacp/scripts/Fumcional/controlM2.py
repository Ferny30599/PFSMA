...
def callback2(data):
    ...
    time = rospy.get_time()
    with open('/home/ferny/smacp/src/smacp/scripts/plot/SMA/evasion/robot2.csv', 'a') as file:
        file.write(f"{x2h}, {y2h}, {time}\n")
...
def control():
    ...
    V = ux*math.cos(th2) + uy*math.sin(th2)
    w = -ux*math.sin(th2)/h + uy*math.cos(th2)/h
    #ux=-k1*(x2h-x1h)-k1*(x2h-x3h)
    #uy=-k1*(y2h-y1h)-k1*(y2h-y3h)
...
def CamposPotenciales():
    ...
    ux=Fa[0]+Fr[0]
    uy=Fa[1]+Fr[1]
    #print(f"(Fax={Fa[0]})+(Frx={Fr[0]})={ux}")
    #print(f"(Fay={Fa[1]})+(Fry={Fr[1]})={uy}")
...
def F_atractiva():
    ...
    xhpos=np.array([x1h, x2h, x3h, x4h, x5h,x6h])-lx
    yhpos=np.array([y1h, y2h, y3h, y4h, y5h,y6h])-ly
    d=math.sqrt((x2h-x1h)**2+(y2h-y1h)**2)
    #if d<Qd:
...
if __name__=="__main__":
    ...
    rospy.Subscriber('/r2/s1', LaserScan, ultrasonic1_callback)
    rospy.Subscriber('/r2/s2', LaserScan, ultrasonic2_callback)
    rospy.Subscriber('/r2/s3', LaserScan, ultrasonic3_callback)
    rospy.Subscriber('/r2/s4', LaserScan, ultrasonic4_callback)
    rospy.Subscriber('/r2/s5', LaserScan, ultrasonic5_callback)
    rospy.Subscriber('/r2/s6', LaserScan, ultrasonic6_callback)
